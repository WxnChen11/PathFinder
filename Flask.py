...
ap_sf = shapefile.Reader("PycharmProjects/Flask/address_points/ADDRESS_POINT_WGS84.dbf")

ap_shapes = ap_sf.shapes()

cl_sf = shapefile.Reader("PycharmProjects/Flask/centreline/CENTRELINE_WGS84.dbf")

cl_shapes = cl_sf.shapes()

clint_sf = shapefile.Reader("PycharmProjects/Flask/centreline_intersection/CENTRELINE_INTERSECTION_WGS84.dbf")
...
def StrPathAstar(start, dest):
    ...
    distGoal = {} #distance from node to end
    previous = {}
    ...
    for i in range(len(clint_records)):
        ...
        distGoal[clint_records[i][0]] = 10000000000000000

        previous[clint_records[i][0]] = -1

    distances[start] = 0

    distGoal[start] = haversine_d(startlat, endlat, startlon, endlon)*.5
    ...
    while len(unvisited) > 0:
        ...
        firstElement = list(unvisited.keys())[0]

        current_lowest = firstElement
        shortestD = distGoal[firstElement]

        for key, value in distGoal.items() :
            if key in visited:
                continue
        ...
        visited[current_lowest] = 0
        del unvisited[current_lowest]

        for e in G.neighbors(current_lowest):
            if e in visited:
                continue

            curr_lat = G.node[current_lowest]["lat"]
            curr_lon = G.node[current_lowest]["lon"]

            testlength = distances[current_lowest] + haversine_d(curr_lat, G.node[e]["lat"] , curr_lon, G.node[e]["lon"])

            if e not in unvisited.keys():
                unvisited[e] = 0

            elif testlength >= distances[e]:
                continue
            ...
            distance_to_goal = testlength + haversine_d(curr_lat, endlat, curr_lon, endlon)*.5
            distGoal[e] = distance_to_goal

        #     if distance_to_goal < distGoal[current_lowest]:
        #         print("new lowest")
        #         next_lowest = e
        #
        # current_lowest = next_lowest
...
def get_page():
    ...
    #return open(r"PycharmProjects/Flask/index.html").read()
    print ("WTf")
    return send_file('PycharmProjects/Flask/templates/index.html')
